...
export class GemmWasmRunner {
  ...
  static async prepare(
    ...
    return new GemmWasmRunner(exports, a, b, c0, timings);
  }

  transfer(): void {
    const start = performance.now();
    new Float32Array(this.exports.memory.buffer, GEMM_A_OFF, this.a.length).set(this.a);
    new Float32Array(this.exports.memory.buffer, GEMM_B_OFF, this.b.length).set(this.b);
    new Float32Array(this.exports.memory.buffer, GEMM_C0_OFF, this.c0.length).set(this.c0);
  ...
  reset(): void {
    const start = performance.now();
    new Uint8Array(this.exports.memory.buffer, GEMM_C_OFF, this.c0.byteLength).set(
      new Uint8Array(this.exports.memory.buffer, GEMM_C0_OFF, this.c0.byteLength),
    );
  ...
  output(): Float32Array {
    return new Float32Array(this.exports.memory.buffer, GEMM_C_OFF, GEMM_BATCH * GEMM_M * GEMM_N)
      .slice();
  }
...
export class GemmJsRunner {
  ...
  // initialize: allocate the owned working buffers (timed).
  static prepare(timings: PhaseTimings): GemmJsRunner {
    const start = performance.now();
    const a = new Float32Array(GEMM_BATCH * GEMM_M * GEMM_K);
    const b = new Float32Array(GEMM_BATCH * GEMM_K * GEMM_N);
    const c0 = new Float32Array(GEMM_BATCH * GEMM_M * GEMM_N);
    const c = new Float32Array(GEMM_BATCH * GEMM_M * GEMM_N);
    timings.initialize += performance.now() - start;
    return new GemmJsRunner(a, b, c0, c, timings);
  }
...
export class MlpWasmRunner {
  ...
  static async prepare(
    ...
    timings.initialize += performance.now() - start;
    const runner = new MlpWasmRunner(exports, timings);
  ...
  transferInputs(x: Float32Array, w: Float32Array, bias: Float32Array): void {
    const start = performance.now();
    new Float32Array(this.exports.memory.buffer, MLP_X_OFF, x.length).set(x);
    new Float32Array(this.exports.memory.buffer, MLP_W_OFF, w.length).set(w);
    new Float32Array(this.exports.memory.buffer, MLP_BIAS_OFF, bias.length).set(bias);
    this.timings.transfer += performance.now() - start;
  }
  ...
  output(): Float32Array {
    return new Float32Array(this.exports.memory.buffer, MLP_Y_OFF, MLP_BATCH * WIDTH).slice();
  }
...
export class MlpJsRunner {
  ...
  // initialize: allocate owned tensors, scratch, and output (timed).
  static prepare(timings: PhaseTimings): MlpJsRunner {
    ...
    const y = new Float32Array(MLP_BATCH * WIDTH);
    timings.initialize += performance.now() - start;
    return new MlpJsRunner(x, w, bias, scratchA, scratchB, y, timings);
  }
