...
- The frozen M1 Chrome experiment uses separate cold and warm headline strata, each with a 20 committed-pair floor, fixed checkpoints 20/30/40/50/60, and a 60-pair cap. Its point estimate is the median per-launch log ratio of Wasm median to JavaScript median. Ten thousand deterministic paired bootstrap resamples use frozen xorshift32 seed/semantics. Five sequential looks use Bonferroni-protected two-sided 99% intervals; a stratum stops when `exp((upperLogCi-lowerLogCi)/2)-1 <= 0.03`, or becomes inconclusive only on reaching 60 without precision. See `experiments/m1-chrome-sum-u32-v1/preregistration.json`.
- Publish the paired ratio and absolute difference with the preregistered interval, plus median, mean, p5/p25/p75/p95/p99, MAD/IQR, count, failures, and raw samples.
