...
Deno.test("base crypto supplement preserves frozen catalog bytes and counts exactly one of 38", async () => {
  assertEquals(
  ...
  assertEquals(ledger.counts, { denominator: 38, implemented: 1, remaining: 37 });
  assertEquals(ledger.implemented.map((entry: { id: string }) => entry.id), [
    "crypto.file-integrity.v1",
  ]);
  assertEquals(ledger.remainingIds.length, 37);
...
Deno.test("retained full validation proves all 36 registered cases and exact counters", async () => {
  ...
  assertEquals(evidence.status, "complete-correctness-validation");
  ...
  assertEquals(
    new Set(evidence.cases.map((entry: { target: string }) => entry.target)),
    new Set(["js-controlled", "wasm-linear-controlled"]),
  );
