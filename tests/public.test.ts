...
Deno.test("workload catalog exposes exact totals, filters, and honest implementation coverage", async () => {
  ...
  assert(page.includes("38 proposed workloads; 49 runnable browser demos"));
  assert(page.includes("49 runnable browser demos across 38 proposed workloads"));
  assert(page.includes("v2 proposal implementation inventory"));
  assert(page.includes("Runnable demos: 14"));
  assert(page.includes("10 full proposal-validation routes and 4 reduced-fixture routes"));
...
Deno.test("public pages contain no inline script, inline style, or remote asset", async () => {
  for (
    ...
    const withoutAllowedLinks = html
      ...
      .replaceAll(
        /https:\/\/github\.com\/PaulKinlan\/wasm-vs-js\/blob\/(?:30b41425227dc139304f25942d0be0d933fa28c9|9691f0e8353a221880f365712a1ebbec18b7dde4|e1e19c5b3f817f9ed42c6d8e2cfc9978c2be53f4|79ae3d65cf4d012acdaac376512f86e26e918080)\/[a-zA-Z0-9._/-]+(?:#L[0-9]+(?:-L[0-9]+)?)?/g,
        "",
      );
