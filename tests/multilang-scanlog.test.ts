...
const PREFIXES = [
  "http://", "https://", "ws://", "wss://", "ftp://", "asset://", "api://", "cdn://",
  "ip=", "client-ip:", "source-ip:", "dest-ip:", "peer-ip:", "origin-ip:",
  "status=", "code=", "http-status:", "response-status:", "result-status:", "status-code:",
];
...
function makeCorpus(records = 40): Uint8Array {
  ...
  for (let record = 0; record < records; record++) {
    ...
    if (record % EVENT_INTERVAL === 0) {
      ...
      let token: string;
      if (MATCHERS[pi] === 1) {
        token = `${PREFIXES[pi]}node-${v.toString(16).padStart(8, "0")}.example.test/path/${eventIndex}`;
      } else if (MATCHERS[pi] === 2) {
        token = `${PREFIXES[pi]}${1 + (v & 0xfe)}.${(v >>> 8) & 0xff}.${(v >>> 16) & 0xff}.${(v >>> 24) & 0xff}`;
      } else {
...
function isUrlTail(b: number): boolean {
  return (b >= 97 && b <= 122) || (b >= 48 && b <= 57) || b === 46 || b === 47 || b === 95 || b === 45;
}

// Exact mirror of workload.js scanControlled for the 20 fixed patterns.
function oracleJS(bytes: Uint8Array): {
  matches: Array<[number, number, number]>; cs: number; pc: number; tc: number;
} {
...
function assertBitIdentical(
  ...
  for (let i = 0; i < matches.length; i++) {
    assert(
      ...
      `${label} match ${i} mismatch: ${JSON.stringify(matches[i])} vs ${JSON.stringify(ref.matches[i])}`,
    );
  }
}
  ...
  async () => {
    ...
    for (const [file, label] of linear) {
      ...
      const count = (mod.instance.exports.scan_log as (
        b: number, l: number, i: number, s: number, e: number, c: number,
        sc: number, cs: number, pc: number, tc: number,
      ) => number)(dataOff, corpus.length, idOff, stOff, enOff, cap, scratchOff, csOff, pcOff, tcOff);
    ...
    const dCount = kernels.scan_log(
      corpus, corpus.length, ids, sts, ends, cap, new Uint32Array(256 * 5), cs, pc, tc,
    );
    const matches: Array<[number, number, number]> = [];
