// protobuf_gateway_kernel.c — multilang compute core for
// serialization.protobuf-gateway.v1.
//
// Ports the frozen wire decoder + filter pass of runJavaScript()
// (benchmarks/base/serialization-protobuf-gateway/workload.js) at the linear
// Wasm layer using the shared multilang-wasm kernel ABI: the adapter writes
// the frozen 1,534,122-byte fixture bytes (framed output of
// generateFixture(): u32 count + [u32 length, wire bytes]×10,000) into linear
// memory at FIXTURE_OFFSET, and the kernel:
//   1. walks every varint tag + payload + length prefix, matching the JS
//      field-and-wire dispatch for fields 1..11 (id, name, active, score,
//      status, tags, metrics, payload, note, code, ratio) and treating any
//      other admitted wire type as an unknown field, exactly as
//      decodeMessage() does;
//   2. applies the filter (active && status !== 3 && id % 3 === 0), the same
//      predicate runJavaScript() uses to select messages for ProtoJSON
//      emission;
//   3. mixes a deterministic 48-byte per-message summary
//      (id_lo, id_hi, active, status, name_len, tag_count, map_count,
//      payload_len, choice_kind, note_len, code_bits, filter_pass) into a
//      running FNV-1a stream so any bit-drift in the wire decode surfaces.
//
// Oracle numbers (measured against decodeMessage() on the frozen fixture):
//   messages         = 10000
//   fields           = 170294
//   varint_bytes     = 474984
//   unknown_fields   = 40000
//   filtered         = 1703
//   summary_fnv1a    = 0xea8c453e
//
// Results (u32 words at RES_OFFSET):
//   [0] messages         (=10000)   [1] fields         (=170294)
//   [2] varint_bytes     (=474984)  [3] unknown_fields (=40000)
//   [4] filtered         (=1703)    [5] wire_bytes     (=1534122)
//   [6] summary_fnv1a    (=0xea8c453e)  [7] status_code (0 = ok)
// Exports:
//   i32 protobuf_gateway(u32 fixture_len) → 0 ok / negative error code.
...
// FIXTURE_OFFSET sits past every language's .bss window (Rust LLD lands
// ~1-2.9 MiB for its rustc-emitted metadata). Total peak memory ~8 MiB.
#define FIXTURE_OFFSET 3145728u   // 3 MiB
#define RES_OFFSET     6291456u   // 6 MiB

#define MESSAGE_COUNT 10000u
...
// readVarint: decodes up to 10 bytes at *cur into (lo, hi). Advances *cur.
// Returns 0 on success (also writes *bytes_used); -1 on truncation; -2 on
// overflow (>10 continuation bytes).
static i32 read_varint(u32 *cur, u32 end, u32 *lo, u32 *hi, u32 *used) {
  ...
  for (u32 i = 0; i < 10u; i++) {
    ...
    (*cur)++;
    bytes++;
    value |= ((u64)(b & 0x7fu)) << shift;
    if (!(b & 0x80u)) {
      *lo = (u32)(value & 0xffffffffu);
      *hi = (u32)(value >> 32);
      *used = bytes;
      return 0;
    }
...
// skip(cur, wire) mirrors JS skip() for the four admitted wire types.
...
// Parse one map entry (field 7 payload): tag/wire dispatch reading a single
// key/value pair. Returns 0 on success and populates *value_lo/*value_hi with
// the decoded sint64 (zigzag).
...
// Message digest state — recomputed per message.
...
// Decodes one message. Advances *cur to the end of the message. Aggregates
// counters into the *_out pointers. Returns 0 on success or a negative error.
static i32 decode_message(
  ...
  while (cur < end) {
    ...
    if (field == 1u && wire == 0u) {
      ...
      // score is a fixed64 (double). We don't consume-decode it into a
      // digest field; the summary tracks only wire structural fields.
      cur += 8u;
    } else if (field == 5u && wire == 0u) {
      u32 lo, hi, u2;
      if (read_varint(&cur, end, &lo, &hi, &u2) != 0) return -9;
      // JS: m.status = Number(BigInt.asIntN(32, v)). Only the low 32 bits.
      m_status = lo;
      varint_bytes += u2;
...
// id % 3 for a 64-bit id (lo, hi). Straight u64 modulo would be enough in C
// with i64 support, but we keep it explicit so every language does the same
// two-word remainder walk (bit-serial mod-3 accumulator).
...
__attribute__((export_name("protobuf_gateway")))
