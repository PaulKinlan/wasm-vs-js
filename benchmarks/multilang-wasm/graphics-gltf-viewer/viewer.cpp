// viewer.cpp — C++ build of the glTF viewer kernel.
//
// graphics.gltf-viewer.v1 had no multi-language comparison at all: one C
// kernel, one JavaScript engine, no manifest and no adapter. This is the same
// program compiled as C++, which is the cheapest honest second engine — the
// source is byte-identical to viewer.c apart from this header and the extern
// "C" linkage the exports need.
