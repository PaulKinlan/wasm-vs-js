// C++ mirror of the frozen cad-mesh-repair engine (bit-identical to
// engine.js repairMeshJavaScript and the C reference). Same ABI as the C
// kernel: input_ptr()/output_ptr()/run(len).
...
static i32 vertex(i32 x, i32 y, i32 z) {
  ...
  verts[vcount * 3] = x; verts[vcount * 3 + 1] = y; verts[vcount * 3 + 2] = z;
  return vcount++;
}
...
extern "C" {
__attribute__((export_name("input_ptr"))) i32 input_ptr() { return (i32)(long)input_data; }
__attribute__((export_name("output_ptr"))) i32 output_ptr() { return (i32)(long)output_data; }
__attribute__((export_name("run"))) i32 run(i32 len) {
  ...
  for (i32 f = 0; f < n; f++) {
    i32 at = 84 + f * 50 + 12;
    for (i32 p = 0; p < 3; p++) {
      i32 x = quant(read_f32(in + at + p * 12));
      i32 y = quant(read_f32(in + at + p * 12 + 4));
      i32 z = quant(read_f32(in + at + p * 12 + 8));
      i32 vid = vertex(x, y, z);
      if (vid < 0) return -3;
      id[p] = vid;
    }
    if (id[0] == id[1] || id[1] == id[2] || id[0] == id[2]) { removed++; continue; }
    i32 ax = verts[id[0] * 3], ay = verts[id[0] * 3 + 1];
    i32 bx = verts[id[1] * 3], by = verts[id[1] * 3 + 1];
    i32 cx = verts[id[2] * 3], cy = verts[id[2] * 3 + 1];
    long long nz = (long long)(bx - ax) * (cy - ay) - (long long)(by - ay) * (cx - ax);
    if (nz == 0) { removed++; continue; }
    if (nz < 0) { i32 sw = id[1]; id[1] = id[2]; id[2] = sw; flipped++; }
    if (fcount >= MAXF) return -4;
    faces[fcount * 3] = id[0]; faces[fcount * 3 + 1] = id[1]; faces[fcount * 3 + 2] = id[2];
    fcount++;
  }
  i32 cleanFaceCount = fcount;
  if (cleanFaceCount % 2 != 0) return -5;
  for (i32 i = 0; i < cleanFaceCount; i++) {
    for (i32 e = 0; e < 3; e++) {
      i32 a = faces[i * 3 + e], b = faces[i * 3 + (e + 1) % 3], incidence = 0;
      for (i32 j = 0; j < cleanFaceCount; j++) {
        for (i32 q = 0; q < 3; q++) {
  ...
  for (i32 i = 0; i < vcount; i++) {
    i32 ox = verts[i * 3];
    i32 x = ((ox < 0 ? -ox : ox) / 10000) % 2 == 1 ? ox - 10000 : ox;
    i32 y = verts[i * 3 + 1], z = verts[i * 3 + 2];
    i32 next = -1;
    for (i32 c = 0; c < sv; c++) {
    ...
    if (next < 0) { next = sv++; simpverts[next * 3] = x; simpverts[next * 3 + 1] = y; simpverts[next * 3 + 2] = z; }
    remap[i] = next;
  }
  i32 target = cleanFaceCount / 2;
  i32 sc = 0;
  for (i32 i = 0; i < cleanFaceCount; i++) {
    i32 a = remap[faces[i * 3]], b = remap[faces[i * 3 + 1]], c = remap[faces[i * 3 + 2]];
    if (a != b && b != c && a != c) {
      selected[sc * 3] = a; selected[sc * 3 + 1] = b; selected[sc * 3 + 2] = c; sc++;
    }
  }
  if (sc != target) return -7;
  for (i32 i = 0; i < sc; i++) {
    for (i32 e = 0; e < 3; e++) {
      i32 a = selected[i * 3 + e], b = selected[i * 3 + (e + 1) % 3], incidence = 0, seen = 0;
      for (i32 j = 0; j < sc; j++) {
        for (i32 q = 0; q < 3; q++) {
