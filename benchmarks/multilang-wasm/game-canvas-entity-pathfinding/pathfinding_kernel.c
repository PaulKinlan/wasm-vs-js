// pathfinding_kernel.c — multilang compute core for
// game.canvas-entity-pathfinding.v1.
//
// ABI (mirrors the gc_document_kernel multilang template): the adapter writes
// the frozen 106,552-byte pathfinding fixture (game-v2-controlled-family/
// game-canvas-entity-pathfinding-v1.bin — 6×u32 header + 256×256 map + 4,096
// entity spawns + 128 path requests + 1,800 u32 controls, generated by
// benchmarks/v2/game-family/fixtures.js with seed 0x8f4c21a7) into linear
// memory at FIXTURE_OFFSET, passes the byte length, and this kernel runs the
// full pathfinding + 1,800-frame ECS loop (bit-for-bit identical to
// run_pathfinding() in benchmarks/v2/game-family/game-family.c and
// pathfinding() in engine.js). We accept the fixture bytes via memory rather
// than regenerating them — the fixture is a 104 KiB seed-derived xorshift
// stream and the authoritative game-family C already reads it the same way,
// so reusing that oracle keeps this kernel byte-identical with
// runGameJavaScript() and runGameWasm().
//
// Results (fixed offset RES_OFFSET, u32 little-endian):
//   [0]  semanticDigest              (0xfe0377bf)
//   [1]  pathNodeSequenceDigest      (0x55c75f61)
//   [2]  tieBreakDigest              (0x8108145d)
//   [3]  ecsCheckpointDigest         (0xc497aa94)
//   [4]  animationCommandStreamDigest(0x569ffa98)
//   [5]  drawCommandStreamDigest     (0x992a9d1d)
//   [6]  audioEventStreamDigest      (0xd1fcc811)
//   [7]  systemUpdates               (7,372,800)
//   [8]  pathNodesExpanded           (974,592)
//   [9]  frontierOperations          (2,213,135)
//   [10] drawCommands                (7,372,800)
//   [11] audioEvents                 (1)
// Exports: i32 pathfinding_trace(u32 fixture_len) -> 0 on success, non-zero
// on fixture-length mismatch or heap overflow.
...
// A* + entity working set (identical layout to run_pathfinding() in
...
__attribute__((export_name("pathfinding_trace")))
int pathfinding_trace(u32 fixture_len) {
  ...
  for (u32 request = 0; request < 128u; request++) {
    ...
    if (
      !heap_push(
        start,
        absolute((i32)(start & 255u) - (i32)gx) + absolute((i32)(start >> 8) - (i32)gy),
        &frontier_operations
      )
    ) return 3;
    ...
    while (heap_length) {
      ...
      expanded++;
      state = mix(state, node ^ (request << 16) ^ (u32)astar_g[node]);
      if (node == goal) break;
      ...
      for (u32 i = 0; i < 4u; i++) {
        ...
        if (astar_seen[next] != stamp || cost < astar_g[next]) {
          ...
          astar_g[next] = cost;
          astar_parent[next] = (i32)node;
          const u32 estimate = (u32)cost +
            absolute((i32)(next & 255u) - (i32)gx) +
    ...
    if (astar_closed[goal] == stamp) {
      i32 node = (i32)goal;
      while (node >= 0) {
        request_path = mix(request_path, (u32)node);
        node = astar_parent[(u32)node];
      }
    } else {
  ...
  for (u32 entity = 0; entity < 4096u; entity++) {
    ...
    entity_x[entity] = (u16)read16(at);
    entity_y[entity] = (u16)read16(at + 2u);
    entity_vx[entity] = (i8)((i32)read16(at + 4u) - 3);
    entity_vy[entity] = (i8)((i32)read16(at + 6u) - 3);
  }
  ...
  for (u32 frame = 0; frame < 1800u; frame++) {
    ...
    for (u32 entity = 0; entity < 4096u; entity++) {
      entity_x[entity] = (u16)(((i32)entity_x[entity] + entity_vx[entity] +
        (i32)(control & 1u) + 256) & 255);
      entity_y[entity] = (u16)(((i32)entity_y[entity] + entity_vy[entity] +
        (i32)((control >> 1) & 1u) + 256) & 255);
      const u32 packed = entity_x[entity] ^ ((u32)entity_y[entity] << 8) ^
        entity ^ control;
      ecs = mix(ecs, packed);
