...
export function generateInput() {
  ...
  for (let layer = 0; layer < LAYERS; layer += 1) {
    ...
    fillUniformF32(
      GENERATOR_SEED,
      stream,
      countValue(w.subarray(layer * WIDTH * WIDTH, (layer + 1) * WIDTH * WIDTH)),
      0.0625,
    );
    stream += 1;
    fillUniformF32(
      GENERATOR_SEED,
      stream,
      countValue(bias.subarray(layer * WIDTH, (layer + 1) * WIDTH)),
      0.25,
    );
    stream += 1;
  }
  return countValue({ x, w, bias });
}
...
// catalog's declared counter set for ml.dense-mlp.v1. allocations counts
// WORKLOAD-OWNED typed-array/object creations per repetition (see
// benchmarks/v2/shared/allocations.js for the scope definition); the
// compute path performs none.
