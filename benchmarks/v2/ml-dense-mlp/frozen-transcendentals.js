import { countValue, createF64 } from "../shared/allocations.js";
...
// exponent-bit scaling at the overflow edge. The table is workload-owned
// and routes through the counting factory; it is created once during module
// evaluation (the load phase), never during compute.
const POW2_TABLE = (() => {
  const table = createF64(2047);
  const buffer = countValue(new ArrayBuffer(8));
  const view = countValue(new DataView(buffer));
