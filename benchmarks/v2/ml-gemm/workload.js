import { countValue, createF32 } from "../shared/allocations.js";
...
export function generateInput() {
  const a = createF32(BATCH * M * K);
  const b = createF32(BATCH * K * N);
  const c0 = createF32(BATCH * M * N);
  let stream = 0;
  for (let t = 0; t < BATCH; t += 1) {
    // Fixture-scope subarray views route through the counting factory:
    // workload-owned allocations are counted wherever they occur.
    fillUniformF32(GENERATOR_SEED, stream, countValue(a.subarray(t * M * K, (t + 1) * M * K)), 1);
    stream += 1;
    fillUniformF32(GENERATOR_SEED, stream, countValue(b.subarray(t * K * N, (t + 1) * K * N)), 1);
    stream += 1;
    fillUniformF32(GENERATOR_SEED, stream, countValue(c0.subarray(t * M * N, (t + 1) * M * N)), 1);
    stream += 1;
  }
  return countValue({ a, b, c0 });
}
...
// loads before cache effects: two per MAC plus one initial-C load per
// output. allocations counts WORKLOAD-OWNED typed-array/object creations per
// repetition (see benchmarks/v2/shared/allocations.js for the scope
// definition); the compute path performs none.
