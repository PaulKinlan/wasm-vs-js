...
static int8_t all_features[HOPS * FEATURES];
static int32_t all_scores[HOPS * CLASSES];
static int32_t detections[HOPS * 3];
...
static void feature_for_hop(int hop, int8_t *out) {
  ...
  for (int i = 0; i < FFT_SIZE; i++) {
    int source = base + i;
    re_buf[i] = i < WINDOW_SAMPLES && source < HOPS * HOP_SAMPLES
  ...
  for (int hop = 0; hop < HOPS; hop++) {
    ...
    if (candidate_count == 5 && candidate != accepted) {
      ...
      if (accepted != 10 && detection_count_value < HOPS) {
        ...
        detections[out] = hop; detections[out + 1] = accepted; detections[out + 2] = scores[accepted];
...
int8_t *features_ptr(void) { return all_features; }
