...
cdp.on("Runtime.consoleAPICalled", (p: unknown) => {
  const params =
    (p as { params?: { type?: string; args?: Array<{ value?: string; description?: string }> } })
      .params;
  if (params?.type === "error") {
    consoleErrs.push(
      params.args?.map((a) => String(a.value ?? a.description ?? "")).join(" ") ?? "",
    );
  }
});
cdp.on("Log.entryAdded", (p: unknown) => {
  const e = (p as { params?: { entry?: { level?: string; text?: string; url?: string } } }).params
    ?.entry;
...
await s.send("Runtime.evaluate", {
  expression:
    `(() => { const card = document.querySelector('[data-slug="sum-u32"]'); const btn = card?.querySelector('.btn-pg-run'); if (btn) { btn.click(); return 'clicked'; } return 'no run btn'; })()`,
});
await new Promise((r) => setTimeout(r, 6000));
const status = await s.send("Runtime.evaluate", {
  expression:
    'document.querySelector(\'[data-slug="sum-u32"] .playground-status\')?.textContent ?? ""',
  returnByValue: true,
});
console.log("DIAG_STATUS:", JSON.stringify(status));
const perf = await s.send("Runtime.evaluate", {
  expression:
    'JSON.stringify(performance.getEntriesByType("resource").map(e => ({name: e.name.slice(-60), dur: Math.round(e.duration), size: e.transferSize})).filter(e => e.size === 0 && !e.name.endsWith("about:blank")))',
  returnByValue: true,
});
