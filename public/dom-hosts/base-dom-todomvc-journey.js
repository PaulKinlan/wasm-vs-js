...
export async function createTodomvcHost() {
  ...
  async function run({ iterations, targets, onProgress = () => {} }) {
    ...
    try {
      ...
      // Keep the final rendered UI in the DOM when the run succeeded so it can
      // be inspected (Paul directive: he must be able to SEE the DOM that was
      // driven). Only remove it if the run failed.
      // (ui.root stays; the parent iframe is kept alive by the bridge keepAlive.)
    }
  }
