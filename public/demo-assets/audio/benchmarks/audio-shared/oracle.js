import { FFT_SIZE, fftRadix2, generateTwiddleTable, TRANSFORMS } from "../audio-fft/workload.js";
import { generateTaps, TAPS } from "../audio-fir/workload.js";
import { FRAME_SIZE, FRAMES, HOP_SIZE, SAMPLE_RATE } from "../audio-stft/workload.js";
import { canonicalF32Bytes, sha256Hex } from "./canonical.js";
import { AUDIO_FROZEN_HASHES } from "./constants.js";
function hybridBound(actual, expected, absolute, relative) {
  return Math.abs(actual - expected) <= Math.max(absolute, relative * Math.abs(expected));
}
export function assertCompleteOutput(actual, expected, absolute, relative) {
...
function assertReconstruction(spectrum, original, n, absolute, relative) {
  const inverse = spectrum.slice();
  for (let i = 0; i < n; i++) {
    inverse[i * 2 + 1] = Math.fround(-inverse[i * 2 + 1]);
  }
...
function f64DirectBin(input, n, bin) {
  let real = 0;
  let imaginary = 0;
...
export function validateFftStructure(input, output) {
  if (output.length !== TRANSFORMS * FFT_SIZE * 2) {
    throw new Error("FFT output length invariant");
  }
...
export function validateFirStructure(output) {
  const expectedLength = 131_072 + TAPS - 1;
  if (output.length !== expectedLength) {
    throw new Error("FIR output length invariant");
  }
  for (let index = 0; index < output.length; index++) {
    if (!Number.isFinite(output[index])) {
      throw new Error(`FIR finite invariant failed at ${index}`);
    }
  }
  ...
  if (Math.abs(gain - 1) > 1e-6) {
    throw new Error(`FIR DC-gain invariant failed: ${gain}`);
  }
  let firstNonZero = -1;
  ...
  if (firstNonZero < 0) {
    throw new Error("FIR impulse-latency invariant has no non-zero tap");
  }
  return { outputLength: output.length, dcGain: gain, impulseLatency: firstNonZero, finite: true };
}
export function validateStftStructure(input, window, output) {
  if (output.length !== FRAMES * FRAME_SIZE * 2) {
    throw new Error("STFT frame-count invariant");
  }
  let symmetryMaximum = 0;
  let reconstructionMaximum = 0;
  ...
  for (let frame = 0; frame < FRAMES; frame++) {
    const spectrum = output.subarray(frame * FRAME_SIZE * 2, (frame + 1) * FRAME_SIZE * 2);
    ...
    if (binError > 1.1) {
      throw new Error(`STFT chirp-bin invariant failed in frame ${frame}`);
    }
  }
  return {
    frames: FRAMES,
...
export async function assertFrozenOutputHash(slug, output) {
