...
export function generateSignal(length = SAMPLES, seed = SEED) {
  let state = seed >>> 0;
...
export function generateTaps(taps = TAPS) {
  ...
  let sum = 0;
  for (let i = 0; i < taps; i++) {
    sum = Math.fround(sum + h[i]);
  }
  for (let i = 0; i < taps; i++) {
    h[i] = Math.fround(h[i] / sum);
  }
  return h;
}
...
export function firDirectConvolutionInto(input, taps, output) {
...
export function firDirectConvolution(input, taps) {
