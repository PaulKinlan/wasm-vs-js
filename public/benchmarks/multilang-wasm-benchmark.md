...
Generated: 2026-08-26T21:18:31.055Z
...
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 83.78 ms            | 1.00×         |
| **Raw WAT** (Handwritten)      | 96 B                | 0.0013 ms               | 54.34 ms            | 1.54×         |
| **AssemblyScript** (asc -O3)   | 94 B                | 0.001 ms               | 54.41 ms            | 1.54×         |
| **C / Wasm** (Clang -nostdlib) | 757 B               | 0.0018 ms               | 40.6 ms            | 2.06×         |
| **C++ / Wasm** (Clang++ -O3)   | 759 B               | 0.0017 ms               | 42.36 ms            | 1.98×         |
| **Rust / Wasm** (rustc -O)     | 498 B               | 0.0011 ms               | 40.63 ms            | 2.06×         |
| **Dart / WasmGC** (dart2wasm)  | 41222 B              | 0.0978 ms               | 147.96 ms           | 0.57×         |

### 2. Fast Fourier Transform Butterfly (`fft-kernel`, 512 elements, 2,000 iterations)
| Language / Toolchain           | Binary Size (bytes) | Cold Instantiation (ms) | Warm Execution (ms) | Speedup vs JS |
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 35.77 ms            | 1.00×         |
| **AssemblyScript** (asc -O3)   | 532 B              | 0.0011 ms               | 8.54 ms            | 4.19×         |
| **C / Wasm** (Clang -nostdlib) | 1149 B              | 0.0019 ms               | 8.05 ms            | 4.44×         |
| **C++ / Wasm** (Clang++ -O3)   | 1151 B              | 0.0019 ms               | 7.86 ms            | 4.55×         |
| **Rust / Wasm** (rustc -O)     | 889 B              | 0.0012 ms               | 7.76 ms            | 4.61×         |
| **Dart / WasmGC** (dart2wasm)  | 41222 B              | 0.0978 ms               | 108.52 ms           | 0.33×         |
...
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (oracle)        | 0 B                 | 4.17 ms        | 1.00× |
| **C / Wasm** (Clang)           | 3077 B              | 2.16 ms         | 1.93× |
| **C++ / Wasm** (Clang++)       | 3079 B              | 2.12 ms        | 1.97× |
| **Rust / Wasm** (rustc)        | 6122 B              | 0.77 ms         | 5.42× |
| **Dart / WasmGC** (dart2wasm)  | 44023 B             | 20.85 ms       | 0.20× |
...
| Language / Toolchain           | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (fround oracle) | 0 B                 | 729.88 ms        | 1.00× |
| **C / Wasm** (Clang)           | 1186 B              | 272.51 ms         | 2.68× |
| **C++ / Wasm** (Clang++)       | 1188 B              | 266.97 ms        | 2.73× |
| **Rust / Wasm** (rustc)        | 926 B              | 273.75 ms         | 2.67× |
| **AssemblyScript / Wasm** (asc) | 213 B             | 367.22 ms        | 1.99× |
| **Dart / WasmGC** (dart2wasm)  | 39079 B             | 11740.83 ms       | 0.06× |
...
| Language / Toolchain           | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (ControlledSha256) | 0 B              | 465.64 ms        | 1.00× |
| **C / Wasm** (Clang)           | 3197 B              | 104.37 ms         | 4.46× |
| **C++ / Wasm** (Clang++)       | 3244 B              | 104.38 ms        | 4.46× |
| **Rust / Wasm** (rustc)        | 5205 B              | 107.18 ms         | 4.34× |
| **AssemblyScript / Wasm** (asc) | 1610 B             | 103.2 ms        | 4.51× |
| **Dart / WasmGC** (dart2wasm)  | 53795 B             | 356.18 ms       | 1.31× |
...
| -------------------------------- | ------------------- | ----------------------- | ------- |
| **Dart / WasmGC** (dart2wasm)    | 41222 B    | 0.0978 ms | 384 |
