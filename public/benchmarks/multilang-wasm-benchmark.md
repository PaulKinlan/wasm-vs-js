...
Generated: 2026-08-05T13:18:25.157Z
...
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 83.45 ms            | 1.00×         |
| **Raw WAT** (Handwritten)      | 96 B                | 0.0013 ms               | 53.99 ms            | 1.55×         |
| **AssemblyScript** (asc -O3)   | 94 B                | 0.001 ms               | 53.73 ms            | 1.55×         |
| **C / Wasm** (Clang -nostdlib) | 757 B               | 0.0018 ms               | 40.71 ms            | 2.05×         |
| **C++ / Wasm** (Clang++ -O3)   | 759 B               | 0.0017 ms               | 44.6 ms            | 1.87×         |
| **Rust / Wasm** (rustc -O)     | 498 B               | 0.0011 ms               | 40.36 ms            | 2.07×         |
| **Dart / WasmGC** (dart2wasm)  | 39613 B              | 0.0935 ms               | 147.26 ms           | 0.57×         |

### 2. Fast Fourier Transform Butterfly (`fft-kernel`, 512 elements, 2,000 iterations)
| Language / Toolchain           | Binary Size (bytes) | Cold Instantiation (ms) | Warm Execution (ms) | Speedup vs JS |
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 36.34 ms            | 1.00×         |
| **AssemblyScript** (asc -O3)   | 2479 B              | 0.0015 ms               | 8.27 ms            | 4.39×         |
| **C / Wasm** (Clang -nostdlib) | 1149 B              | 0.0018 ms               | 8.09 ms            | 4.49×         |
| **C++ / Wasm** (Clang++ -O3)   | 1151 B              | 0.0018 ms               | 7.8 ms            | 4.66×         |
| **Rust / Wasm** (rustc -O)     | 889 B              | 0.0011 ms               | 7.74 ms            | 4.70×         |
| **Dart / WasmGC** (dart2wasm)  | 39613 B              | 0.0935 ms               | 52.56 ms           | 0.69×         |
...
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (oracle)        | 0 B                 | 2.95 ms        | 1.00× |
| **C / Wasm** (Clang)           | 3077 B              | 2.31 ms         | 1.28× |
| **C++ / Wasm** (Clang++)       | 3079 B              | 1.99 ms        | 1.48× |
| **Rust / Wasm** (rustc)        | 6110 B              | 0.7 ms         | 4.21× |
| **Dart / WasmGC** (dart2wasm)  | 44023 B             | 24.2 ms       | 0.12× |
...
| Language / Toolchain           | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (fround oracle) | 0 B                 | 740.85 ms        | 1.00× |
| **C / Wasm** (Clang)           | 1186 B              | 270 ms         | 2.74× |
| **C++ / Wasm** (Clang++)       | 1188 B              | 267.71 ms        | 2.77× |
| **Rust / Wasm** (rustc)        | 926 B              | 266.68 ms         | 2.78× |
| **Dart / WasmGC** (dart2wasm)  | 39079 B             | 11638.76 ms       | 0.06× |
...
| Language / Toolchain           | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (ControlledSha256) | 0 B              | 476.77 ms        | 1.00× |
| **C / Wasm** (Clang)           | 3197 B              | 105 ms         | 4.54× |
| **C++ / Wasm** (Clang++)       | 3244 B              | 104.9 ms        | 4.54× |
| **Rust / Wasm** (rustc)        | 5189 B              | 107.62 ms         | 4.43× |
| **Dart / WasmGC** (dart2wasm)  | 53795 B             | 357.88 ms       | 1.33× |
