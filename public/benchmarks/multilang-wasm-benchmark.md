...
Generated: 2026-08-05T13:45:30.635Z
...
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 82.51 ms            | 1.00×         |
| **Raw WAT** (Handwritten)      | 96 B                | 0.0013 ms               | 53.68 ms            | 1.54×         |
| **AssemblyScript** (asc -O3)   | 94 B                | 0.001 ms               | 53.5 ms            | 1.54×         |
| **C / Wasm** (Clang -nostdlib) | 757 B               | 0.0018 ms               | 43.62 ms            | 1.89×         |
| **C++ / Wasm** (Clang++ -O3)   | 759 B               | 0.0018 ms               | 42.36 ms            | 1.95×         |
| **Rust / Wasm** (rustc -O)     | 498 B               | 0.0011 ms               | 40.31 ms            | 2.05×         |
| **Dart / WasmGC** (dart2wasm)  | 39613 B              | 0.0935 ms               | 147.35 ms           | 0.56×         |

### 2. Fast Fourier Transform Butterfly (`fft-kernel`, 512 elements, 2,000 iterations)
| Language / Toolchain           | Binary Size (bytes) | Cold Instantiation (ms) | Warm Execution (ms) | Speedup vs JS |
| ------------------------------ | ------------------- | ----------------------- | ------------------- | ------------- |
| **JavaScript** (V8 JIT)        | 0 B                 | 0.00 ms                 | 36.15 ms            | 1.00×         |
| **AssemblyScript** (asc -O3)   | 2479 B              | 0.0015 ms               | 8.32 ms            | 4.34×         |
| **C / Wasm** (Clang -nostdlib) | 1149 B              | 0.0018 ms               | 8.07 ms            | 4.48×         |
| **C++ / Wasm** (Clang++ -O3)   | 1151 B              | 0.0018 ms               | 7.86 ms            | 4.60×         |
| **Rust / Wasm** (rustc -O)     | 889 B              | 0.0011 ms               | 7.79 ms            | 4.64×         |
| **Dart / WasmGC** (dart2wasm)  | 39613 B              | 0.0935 ms               | 52.53 ms           | 0.69×         |
...
| **C / Wasm** (Clang)           | 3077 B              | 2.2 ms         | 1.31× |
| **C++ / Wasm** (Clang++)       | 3079 B              | 2.15 ms        | 1.34× |
| **Rust / Wasm** (rustc)        | 6138 B              | 0.74 ms         | 3.89× |
| **Dart / WasmGC** (dart2wasm)  | 44023 B             | 22.38 ms       | 0.13× |
...
| **C / Wasm** (Clang)           | 1186 B              | 267.12 ms         | 2.75× |
| **C++ / Wasm** (Clang++)       | 1188 B              | 264.51 ms        | 2.77× |
| **Rust / Wasm** (rustc)        | 926 B              | 263.1 ms         | 2.79× |
| **Dart / WasmGC** (dart2wasm)  | 39079 B             | 11946.99 ms       | 0.06× |
...
| Language / Toolchain           | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| ------------------------------ | ------------------- | ------------------- | ----- |
| **JavaScript** (ControlledSha256) | 0 B              | 448.79 ms        | 1.00× |
| **C / Wasm** (Clang)           | 3197 B              | 107.32 ms         | 4.18× |
| **C++ / Wasm** (Clang++)       | 3244 B              | 106.72 ms        | 4.21× |
| **Rust / Wasm** (rustc)        | 5221 B              | 107.75 ms         | 4.17× |
| **Dart / WasmGC** (dart2wasm)  | 53795 B             | 427.38 ms       | 1.05× |
...
| -------------------------------- | ------------------- | ----------------------- | ------- |
| **Dart / WasmGC** (dart2wasm)    | 39613 B    | 0.0935 ms | 380 |
| **Kotlin / Wasm** (prebuilt)     | 36961 B (~37 KB) | 0.0889 ms | 216 |
