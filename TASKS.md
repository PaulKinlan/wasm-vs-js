...
- [ ] **DOM-family real-DOM orchestration (iframe mode) — Paul's core vision**: the DOM workloads exist to measure JS vs Wasm interacting with the REAL DOM (which is better/faster at web work), not just engine state machines. Build iframe-based orchestration: the suite runner loads each DOM demo page (todomvc-journey, virtualized-grid, grid-movement, keyed-list-mutation, nested-tree-mutation, table-sort-filter-pagination, dependent-form-validation, virtualized-scrolling, tactics-grid, flood-fill/image) inside an iframe, posts a start message, the page runs its real-DOM benchmark (JS + Wasm variants applied to the actual DOM), and posts results back. DOM pages gain a postMessage start-listener; the homepage suite runs these in iframe mode instead of worker mode. Engine-level multilang (todomvc worker) complements this — the real-DOM run is the primary measurement.
- [ ] **Report aggregation gap (tracked)**: the multilang report (public/data/multilang-wasm-benchmark-report.v1.json) is missing report rows for simulation-rigid-body-2d and graphics-cpu-path-tracer — their builder registrations (compile + measurement + report-entry blocks in scripts/build-multilang-wasm-benchmark.ts) were lost in 3-way merges. The comparisons ARE live on the pages (adapters/manifests/wiring verified). Fix = add the two registrations to the builder with the documented traps: extract from feat/multilang-rigid-fft-regex-vdom + feat/multilang-path-tracer-dart; rename colliding consts (GEMM_, report, sumVariants deps); dedupe watSumBytes; add the workload imports; clean insertion boundaries; regen report → expect 25 workloads.
