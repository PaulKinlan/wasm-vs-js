...
if (!/^[a-f0-9]{40}$/.test(sourceCommit) || !chromeExecutable || Deno.args.length !== 2) {
  throw new Error(
    "usage: collect-traditional-demo-evidence.ts --source-commit=<40 hex> --chrome=<path>",
  );
}
...
const screenshotDir = new URL("artifacts/demos/traditional/browser-evidence/screenshots/", root);
await Deno.remove(new URL("artifacts/demos/traditional/browser-evidence/", root), {
  recursive: true,
}).catch(() => {});
...
async function pageState(client: CdpClient, sessionId: string) {
  const evaluated = await client.send("Runtime.evaluate", {
    expression:
      `(() => ({status:document.querySelector('#status').textContent.trim(),result:document.querySelector('#result').textContent.trim(),startDisabled:document.querySelector('#start').disabled,cancelDisabled:document.querySelector('#cancel').disabled}))()`,
    returnByValue: true,
  }, sessionId);
...
const profilePath = await Deno.makeTempDir({ prefix: "wasm-traditional-chrome-" });
