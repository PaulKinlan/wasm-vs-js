...
// Cache-busting counter so the load phase's dynamic import REALLY
// re-evaluates the workload module (including its power-table init) on
// every build instead of returning the cached module instantly.
let loadBust = 0;
...
async function buildGemmArtifacts(): Promise<void> {
  ...
  // load: read the payload source AND evaluate the module for real
  // (cache-busted dynamic import re-runs module evaluation).
  await timedPhase(jsTimings, "load", async () => {
    await Deno.readFile(new URL("benchmarks/v2/ml-gemm/workload.js", root));
    await import(
      new URL(`../benchmarks/v2/ml-gemm/workload.js?load=${++loadBust}`, import.meta.url).href
    );
  });
  ...
  // load: WAT assembly starts INSIDE the load timer.
  const wasm = await timedPhase(
    wasmTimings,
    "load",
    () => assembleWat("ml-gemm.wat", "benchmarks/v2/ml-gemm/ml-gemm.wat"),
  );
  ...
  // Oracle checks: complete-output-bound, structural-invariants,
  // work-counters. EVERY bound, finite, structural, and counter check runs
  // inside the per-target validation phase; nothing validates untimed. The
  // cross-target check is attributed to the Wasm variant's validation.
  // Every oracle check AND every pass/fail decision runs inside the
  // per-target validation phase; nothing validates or decides untimed.
  const jsValidation = await timedPhase(jsTimings, "validation", () => {
    const boundResult = boundCheck(jsOut, reference, bound);
    const health = checkFiniteAndZero(jsOut);
    const structural = gemmStructuralChecks(jsOut, reference, bound);
    const counters = gemmWorkCounters("javascript");
    if (boundResult.maxBoundRatio >= 1) {
      throw new Error(
        `ml-gemm js exceeds stored hybrid bound (ratio ${boundResult.maxBoundRatio})`,
      );
    }
    if (!health.finite) throw new Error("ml-gemm js NaN policy violated");
    if (!health.negativeZeroFree) throw new Error("ml-gemm js signed-zero policy violated");
    for (const check of structural) {
      if (!check.passed) {
        throw new Error(`ml-gemm js structural invariant failed: ${check.id} (${check.detail})`);
      }
    }
    for (const value of Object.values(counters)) {
      if (!Number.isSafeInteger(value) || (value as number) < 0) {
        throw new Error("ml-gemm js work counter is not an exact non-negative integer");
      }
    }
    return { boundResult, structural, counters };
  });
  const wasmValidation = await timedPhase(wasmTimings, "validation", () => {
    const boundResult = boundCheck(wasmOut, reference, bound);
    const health = checkFiniteAndZero(wasmOut);
    const structural = gemmStructuralChecks(wasmOut, reference, bound);
    const counters = gemmWorkCounters("wasm-linear");
    const crossTargetResult = boundCheck(jsOut, Float64Array.from(wasmOut), bound);
    if (boundResult.maxBoundRatio >= 1) {
      throw new Error(
        `ml-gemm wasm exceeds stored hybrid bound (ratio ${boundResult.maxBoundRatio})`,
      );
    }
    if (!health.finite) throw new Error("ml-gemm wasm NaN policy violated");
    if (!health.negativeZeroFree) throw new Error("ml-gemm wasm signed-zero policy violated");
    for (const check of structural) {
      if (!check.passed) {
        throw new Error(`ml-gemm wasm structural invariant failed: ${check.id} (${check.detail})`);
      }
    }
    for (const value of Object.values(counters)) {
      if (!Number.isSafeInteger(value) || (value as number) < 0) {
        throw new Error("ml-gemm wasm work counter is not an exact non-negative integer");
      }
    }
    if (crossTargetResult.maxBoundRatio >= 1) {
      throw new Error("ml-gemm cross-target deviation exceeds stored bound");
    }
    return { boundResult, structural, counters, crossTargetResult };
  });
  const jsBound = jsValidation.boundResult;
  const wasmBound = wasmValidation.boundResult;
  const jsStructural = jsValidation.structural;
  const wasmStructural = wasmValidation.structural;
  const jsCounters = jsValidation.counters;
  const wasmCounters = wasmValidation.counters;
  const crossTarget = wasmValidation.crossTargetResult;
...
async function buildMlpArtifacts(): Promise<void> {
  ...
  const jsTimings = emptyPhaseTimings();
  // load: read the payload source AND evaluate the module for real
  // (cache-busted dynamic import re-runs module evaluation, including the
  // frozen power-table initialization).
  await timedPhase(jsTimings, "load", async () => {
    await Deno.readFile(new URL("benchmarks/v2/ml-dense-mlp/workload.js", root));
    await import(
      new URL(`../benchmarks/v2/ml-dense-mlp/workload.js?load=${++loadBust}`, import.meta.url).href
    );
    // The workload's frozen-transcendental dependency is evaluated at
    // startup by the build's own static import, so cache-busting the
    // workload alone would not re-evaluate it. Import the dependency with
    // its own cache-bust inside the load phase so the power-table
    // initialization is genuinely re-executed and timed.
    await import(
      new URL(
        `../benchmarks/v2/ml-dense-mlp/frozen-transcendentals.js?load=${++loadBust}`,
        import.meta.url,
      ).href
    );
  });
  ...
  const wasm = await timedPhase(
    wasmTimings,
    "load",
    () => assembleWat("ml-dense-mlp.wat", "benchmarks/v2/ml-dense-mlp/ml-dense-mlp.wat"),
  );
  ...
  // Every oracle check, trajectory generation, AND pass/fail decision runs
  // inside the per-target validation phase; nothing validates or decides
  // untimed. The cross-target check is attributed to the Wasm variant.
  // The JS per-layer trajectory AND the oracle derivation (pinned f64
  // reference + measured-deviation bounds) run inside the JS validation
  // phase; the captured reference and bounds are passed forward to the
  // Wasm variant's validation and to artifact serialization. No controlled
  // trajectory executes outside a validation timer anywhere in the build.
  const jsValidation = await timedPhase(jsTimings, "validation", () => {
    const layers = mlpJsLayerOutputs(x, w, bias);
    const reference = mlpReference(x, w, bias, layers);
    const layerChecks = [];
    for (let layer = 0; layer < mlp.LAYERS; layer += 1) {
      const check = boundCheck(layers[layer], reference.references[layer], reference.bounds[layer]);
      const health = checkFiniteAndZero(layers[layer]);
      if (check.maxBoundRatio >= 1) {
        throw new Error(`ml-dense-mlp js layer ${layer} exceeds stored hybrid bound`);
      }
      if (!health.finite) throw new Error(`ml-dense-mlp js NaN policy violated at layer ${layer}`);
      if (!health.negativeZeroFree) {
        throw new Error(`ml-dense-mlp js signed-zero policy violated at layer ${layer}`);
      }
      layerChecks.push({ layer, check });
    }
    const structural = mlpStructuralChecks(
      layers,
    ...
    for (const check of structural) {
      if (!check.passed) {
        throw new Error(
          `ml-dense-mlp js structural invariant failed: ${check.id} (${check.detail})`,
        );
      }
    }
    const counters = mlpWorkCounters("javascript");
    for (const value of Object.values(counters)) {
      if (!Number.isSafeInteger(value) || (value as number) < 0) {
        throw new Error("ml-dense-mlp js work counter is not an exact non-negative integer");
      }
    }
    return { layerChecks, structural, counters, reference };
  });
  const { references, pres, bounds } = jsValidation.reference;
  // Serialize the captured oracle bytes for the artifact set (pure data
  // transformation of validated values, no trajectory or decision).
  ...
  const wasmValidation = await timedPhase(wasmTimings, "validation", () => {
    const layers = mlpWasmLayerOutputs(wasmRunner.exports);
    const layerChecks = [];
    for (let layer = 0; layer < mlp.LAYERS; layer += 1) {
      const check = boundCheck(layers[layer], references[layer], bounds[layer]);
      const health = checkFiniteAndZero(layers[layer]);
      if (check.maxBoundRatio >= 1) {
        throw new Error(`ml-dense-mlp wasm layer ${layer} exceeds stored hybrid bound`);
      }
      if (!health.finite) {
        throw new Error(`ml-dense-mlp wasm NaN policy violated at layer ${layer}`);
      }
      if (!health.negativeZeroFree) {
        throw new Error(`ml-dense-mlp wasm signed-zero policy violated at layer ${layer}`);
      }
      layerChecks.push({ layer, check });
    }
    const structural = mlpStructuralChecks(layers, references, pres, bounds);
    for (const check of structural) {
      if (!check.passed) {
        throw new Error(
          `ml-dense-mlp wasm structural invariant failed: ${check.id} (${check.detail})`,
        );
      }
    }
    const counters = mlpWorkCounters("wasm-linear");
    for (const value of Object.values(counters)) {
      if (!Number.isSafeInteger(value) || (value as number) < 0) {
        throw new Error("ml-dense-mlp wasm work counter is not an exact non-negative integer");
      }
    }
    const crossTargetResult = boundCheck(
      jsRunner.output(),
    ...
    return { layerChecks, structural, counters, crossTargetResult, layers };
  });
  const wasmLayers = wasmValidation.layers;
  const layerChecks: unknown[] = [];
  for (let layer = 0; layer < mlp.LAYERS; layer += 1) {
    layerChecks.push({
      layer,
      js: jsValidation.layerChecks[layer].check,
      wasm: wasmValidation.layerChecks[layer].check,
    });
  }
  const crossTarget = wasmValidation.crossTargetResult;
  const jsStructural = jsValidation.structural;
...
async function buildRecords(): Promise<void> {
  ...
  // Operative dependencies imported by the workloads and the build recipe
  // must be inside the provenance closure (hashes, URLs, collision guards).
  const allocationsSupport = {
    role: "shared-support",
    ...await fileRefGit("benchmarks/v2/shared/allocations.js"),
  };
  const evidenceSupport = {
    role: "shared-support",
    ...await fileRefGit("lib/v2/manifest-evidence.ts"),
  };
