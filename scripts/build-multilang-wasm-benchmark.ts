...
await run("clang", [
  ...
  "-Wl,--no-entry", "-Wl,--export-all", "-Wl,--initial-memory=1048576",
  "-o", `${artifactsDir}/telemetry_c.wasm`, `${rootDir}/benchmarks/multilang-wasm/serialization-json-telemetry/telemetry.c`,
], "compile telemetry C");
await run("clang++", [
  "--target=wasm32", "-O3", "-nostdlib",
  "-Wl,--no-entry", "-Wl,--export-all", "-Wl,--initial-memory=1048576",
  "-o", `${artifactsDir}/telemetry_cpp.wasm`, `${rootDir}/benchmarks/multilang-wasm/serialization-json-telemetry/telemetry.cpp`,
], "compile telemetry C++");
await run("rustc", [
  "--target=wasm32-unknown-unknown", "-O", "--crate-type", "cdylib",
  "-C", "strip=symbols",
  "-o", `${artifactsDir}/telemetry_rs.wasm`, `${rootDir}/benchmarks/multilang-wasm/serialization-json-telemetry/telemetry.rs`,
...
await run("dart", [
  ...
  "-o", `${artifactsDir}/telemetry_dart.wasm`,
...
const jsonFixture = (() => {
  ...
  let st = 0x7e1e2026;
  const xorshift = () => { st ^= st << 13; st ^= st >>> 17; st ^= st << 5; return st >>> 0; };
  ...
  for (let i = 0; i < JSON_RECORDS; i++) {
    ...
    chunks.push(enc.encode(`${i ? "," : ""}{"id":${i},"ts":${ts},"region":"${region}","kind":"${kind}","ok":${ok},"value":${value},"meta":{"label":"${label}","tag":"${tag}"}}`));
  }
...
{
  const jsFn = () => { jsonOracle(jsonFixture, { onAllocation: () => {} }); };
  for (let i = 0; i < 3; i++) jsFn();
  t0 = performance.now();
...
const report = {
  ...
  summary: {
    totalVariantsTested: 7 + 6 + 5 + 5 + 5 + 5 + 5 + 6 + 5 + 5 + 2,
    keyInsights: [
