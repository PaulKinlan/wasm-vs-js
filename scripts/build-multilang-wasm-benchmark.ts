...
const report = {
  ...
  summary: {
    totalVariantsTested: 7 + 6 + 5 + 5 + 5 + 5 + 6 + 5 + 5 + 2 + 6,
    keyInsights: [
...
| Language / Toolchain              | Binary Size (bytes) | Warm Execution (ms) | vs JS |
| --------------------------------- | ------------------- | ------------------- | ----- |
| **JavaScript** (integer oracle)   | 0 B                 | ${imgVariants.js} ms        | 1.00× |
| **C / Wasm** (Clang)              | ${imgBytes.c.byteLength} B              | ${imgVariants.c} ms         | ${
  (imgVariants.js / imgVariants.c).toFixed(2)
}× |
| **C++ / Wasm** (Clang++)          | ${imgBytes.cpp.byteLength} B              | ${imgVariants.cpp} ms        | ${
  (imgVariants.js / imgVariants.cpp).toFixed(2)
}× |
| **Rust / Wasm** (rustc)           | ${imgBytes.rs.byteLength} B              | ${imgVariants.rs} ms         | ${
  (imgVariants.js / imgVariants.rs).toFixed(2)
}× |
| **AssemblyScript** (asc -O3)      | ${imgBytes.asc.byteLength} B              | ${imgVariants.asc} ms        | ${
  (imgVariants.js / imgVariants.asc).toFixed(2)
}× |
| **Dart / WasmGC** (dart2wasm)     | ${imgBytes.dart.byteLength} B             | ${imgVariants.dart} ms       | ${
  (imgVariants.js / imgVariants.dart).toFixed(2)
}× |
...
const manifestSources = [
  ...
  "benchmarks/multilang-wasm/image-editing/image_kernels.c",
  "benchmarks/multilang-wasm/image-editing/image_kernels.cpp",
  "benchmarks/multilang-wasm/image-editing/image_kernels.rs",
  "benchmarks/multilang-wasm/image-editing/image_kernels.ts",
  "benchmarks/multilang-wasm/image-editing/image_kernels.dart",
  "scripts/build-multilang-wasm-benchmark.ts",
  ...
  "tests/multilang-image-editing.test.ts",
