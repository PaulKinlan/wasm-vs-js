...
import { generateRigidBodyFixture } from "../benchmarks/v1/simulation-rigid-body-2d/fixture.js";
...
console.log("Compiling simulation-rigid-body-2d rigid_engine variants (C/C++/Rust/Dart)...");
{
  const src = `${rootDir}/benchmarks/multilang-wasm/simulation-rigid-body-2d`;
  await run("clang", [
    "--target=wasm32",
    "-O3",
    "-nostdlib",
    ...
    "-Wl,--initial-memory=16777216",
    "-o",
    `${artifactsDir}/rigid_c.wasm`,
    `${src}/rigid_engine.c`,
  ], "compile rigid_engine C");
  await run("clang++", [
    "--target=wasm32",
    "-O3",
    "-nostdlib",
    ...
    "-Wl,--initial-memory=16777216",
    "-o",
    `${artifactsDir}/rigid_cpp.wasm`,
    `${src}/rigid_engine.cpp`,
  ], "compile rigid_engine C++");
  await run("rustc", [
    "--target=wasm32-unknown-unknown",
    "-O",
    "--crate-type=cdylib",
    "-o",
    `${artifactsDir}/rigid_rs.wasm`,
    `${src}/rigid_engine.rs`,
  ], "compile rigid_engine Rust");
  await run("dart", [
    "compile",
    "wasm",
    "--no-source-maps",
    `${src}/rigid_engine.dart`,
    "-o",
    `${artifactsDir}/rigid_dart.wasm`,
  ], "compile rigid_engine Dart WasmGC");
}
for (const extra of ["rigid_dart.wasm.map", "rigid_dart.support.js"]) {
  try {
    await Deno.remove(`${artifactsDir}/${extra}`);
  } catch { /* absent */ }
}
const rigidGluePath = `${artifactsDir}/rigid_dart.mjs`;
const rigidGlueText = await Deno.readTextFile(rigidGluePath);
if (!rigidGlueText.startsWith("// deno-lint-ignore-file")) {
  await Deno.writeTextFile(
    rigidGluePath,
    `// deno-lint-ignore-file -- generated by dart2wasm (dart compile wasm)\n${rigidGlueText}`,
  );
...
// ---------------------------------------------------------------------------
// 6i. simulation-rigid-body-2d rigid_engine benchmark (500 bodies, 1800
//      timesteps, checkpoint every 300; 30 warm iterations)
// ---------------------------------------------------------------------------
const RIGID_TIMESTEPS = 300, RIGID_EVERY = 100; // gate shape (3 checkpoints)
const RIGID_M_STEPS = 120, RIGID_M_EVERY = 60, RIGID_ITERATIONS = 30; // reduced measure shape
const RIGID_STATE = 500 * 6, RIGID_WORDS = (RIGID_TIMESTEPS / RIGID_EVERY) * RIGID_STATE;

const rigidLinear = ["c", "cpp", "rs"] as const;
const rigidMods: Record<string, WebAssembly.Instance> = {};
for (const key of rigidLinear) {
  rigidMods[key] = await instantiateLinear(
    await Deno.readFile(`${artifactsDir}/rigid_${key}.wasm`),
  );
}
const { kernels: rigidDart } = await instantiateDartGlue<{
  run: (fixture: Uint8Array, timesteps: number, every: number) => number;
  counters: () => Uint8Array;
  checkpoints: () => Uint8Array;
}>("rigid_dart.mjs", "rigid_dart.wasm");

function rigidLinearFn(key: string): () => void {
  const inst = rigidMods[key];
  const mem = inst.exports.memory as WebAssembly.Memory;
  const fp = (inst.exports.fixture_ptr as unknown as () => number)();
  const rp = (inst.exports.result_ptr as unknown as () => number)();
  const fixture = generateRigidBodyFixture();
  return () => {
    new Uint8Array(mem.buffer, fp, fixture.byteLength).set(
      new Uint8Array(fixture.buffer, fixture.byteOffset, fixture.byteLength),
    );
    (inst.exports.run as unknown as (t: number, e: number) => number)(RIGID_M_STEPS, RIGID_M_EVERY);
    void rp;
  };
}
function rigidDartFn(): () => void {
  const fixture = generateRigidBodyFixture();
  return () => {
    rigidDart.run(
      new Uint8Array(fixture.buffer, fixture.byteOffset, fixture.byteLength),
...
function rigidJsFn(): () => void {
  const fixture = generateRigidBodyFixture();
  return () => {
    runRigidBodyJavaScript(fixture, { timesteps: RIGID_M_STEPS, checkpointEvery: RIGID_M_EVERY });
  };
}

// Bit-identity gate: run every engine once on the same fixture and compare
// the checkpoint stream + the 13 counters against the JS oracle.
const rigidFixture = generateRigidBodyFixture();
const oracle = runRigidBodyJavaScript(rigidFixture, {
  timesteps: RIGID_TIMESTEPS,
  checkpointEvery: RIGID_EVERY,
});
const oracleCheckpoints = oracle.checkpoints ?? [];
function rigidCheckpoints(key: string): Float32Array {
  if (key === "js") return Float32Array.from(oracleCheckpoints as number[]);
  if (key === "dart") {
    const df = new Uint8Array(
      rigidFixture.buffer,
      rigidFixture.byteOffset,
      rigidFixture.byteLength,
    );
    rigidDart.run(df, RIGID_TIMESTEPS, RIGID_EVERY);
    return new Float32Array(rigidDart.checkpoints());
  }
  const inst = rigidMods[key];
  const mem = inst.exports.memory as WebAssembly.Memory;
  const rp = (inst.exports.result_ptr as unknown as () => number)();
  new Uint8Array(
    mem.buffer,
    (inst.exports.fixture_ptr as unknown as () => number)(),
    rigidFixture.byteLength,
  ).set(
    new Uint8Array(rigidFixture.buffer, rigidFixture.byteOffset, rigidFixture.byteLength),
  );
  (inst.exports.run as unknown as (t: number, e: number) => number)(RIGID_TIMESTEPS, RIGID_EVERY);
  return new Float32Array(mem.buffer, rp + 64, RIGID_WORDS);
}
for (const key of ["c", "cpp", "rs", "dart"] as const) {
  const cp = rigidCheckpoints(key);
  if (cp.length !== oracleCheckpoints.length) {
    throw new Error(
      `rigid ${key}: checkpoint length ${cp.length} != oracle ${oracleCheckpoints.length}`,
    );
  }
  for (let i = 0; i < cp.length; i++) {
    if (cp[i] !== oracleCheckpoints[i]) {
      throw new Error(
        `rigid ${key}: checkpoint mismatch at ${i} (${cp[i]} != ${oracleCheckpoints[i]})`,
      );
    }
  }
}

const rigidVariants: Record<string, number> = {};
{
  const jsFn = rigidJsFn();
  for (let i = 0; i < 3; i++) jsFn();
  const tA = performance.now();
  for (let i = 0; i < RIGID_ITERATIONS; i++) jsFn();
  rigidVariants.js = Number((performance.now() - tA).toFixed(2));

  for (const key of rigidLinear) {
    const fn = rigidLinearFn(key);
    for (let i = 0; i < 3; i++) fn();
    const tB = performance.now();
    for (let i = 0; i < RIGID_ITERATIONS; i++) fn();
    rigidVariants[key] = Number((performance.now() - tB).toFixed(2));
  }

  const dartFn = rigidDartFn();
  for (let i = 0; i < 3; i++) dartFn();
  const tD = performance.now();
  for (let i = 0; i < RIGID_ITERATIONS; i++) dartFn();
  rigidVariants.dart = Number((performance.now() - tD).toFixed(2));
}
...
const report = {
  ...
  workloads: [
    ...
    {
      name: "simulation-rigid-body-2d",
      description:
        "500-body 2D physics: SAT rectangle collision, distance joints, torque, quantized " +
        "state, and 6 checkpoints over 1,800 timesteps. All variants are bit-identical to the " +
        "workload's engine.js oracle (checkpoints + all 13 counters, test-verified). Dart " +
        "emulates f32 with per-op rounding (no f32 primitive in WasmGC) — disclosed, not hidden.",
      variants: [
        {
          language: "JavaScript",
          toolchain: "V8 JIT Engine",
          binarySizeBytes: 0,
          coldInstantiateMs: 0,
          warmExecutionMs: rigidVariants.js,
          memoryPageCount: 0,
          importsCount: 0,
          exportsCount: 0,
          notes: "engine.js runRigidBodyJavaScript oracle reference.",
        },
        {
          language: "C / Wasm",
          toolchain: "clang --target=wasm32 -O3 -nostdlib -ffp-contract=off",
          binarySizeBytes: (await Deno.stat(`${artifactsDir}/rigid_c.wasm`)).size,
          coldInstantiateMs: benchmarkColdInstantiate(
            await Deno.readFile(`${artifactsDir}/rigid_c.wasm`),
          ),
          warmExecutionMs: rigidVariants.c,
          memoryPageCount: 1,
          importsCount: 0,
          exportsCount: 3,
          notes: "Bit-identical to the oracle; frozen C kernel is the workload reference.",
        },
        {
          language: "C++ / Wasm",
          toolchain: "clang++ --target=wasm32 -O3 -nostdlib -ffp-contract=off",
          binarySizeBytes: (await Deno.stat(`${artifactsDir}/rigid_cpp.wasm`)).size,
          coldInstantiateMs: benchmarkColdInstantiate(
            await Deno.readFile(`${artifactsDir}/rigid_cpp.wasm`),
          ),
          warmExecutionMs: rigidVariants.cpp,
          memoryPageCount: 1,
          importsCount: 0,
          exportsCount: 3,
          notes: "Same algorithm as the C variant (bit-identical output).",
        },
        {
          language: "Rust / Wasm",
          toolchain: "rustc --target=wasm32-unknown-unknown -O --crate-type=cdylib",
          binarySizeBytes: (await Deno.stat(`${artifactsDir}/rigid_rs.wasm`)).size,
          coldInstantiateMs: benchmarkColdInstantiate(
            await Deno.readFile(`${artifactsDir}/rigid_rs.wasm`),
          ),
          warmExecutionMs: rigidVariants.rs,
          memoryPageCount: 1,
          importsCount: 0,
          exportsCount: 3,
          notes: "no_std; f32 sqrt via the f32.sqrt instruction (LLVM intrinsic lowering).",
        },
        {
          language: "Dart / WasmGC",
          toolchain: "dart compile wasm (dart2wasm, Dart 3.12.2)",
          binarySizeBytes: (await Deno.stat(`${artifactsDir}/rigid_dart.wasm`)).size,
          coldInstantiateMs: benchmarkColdInstantiate(
            await Deno.readFile(`${artifactsDir}/rigid_dart.wasm`),
            JS_STRING_BUILTINS,
          ),
          warmExecutionMs: rigidVariants.dart,
          memoryPageCount: 2,
          importsCount: 0,
          exportsCount: 3,
          notes: "f32 emulated with per-op rounding — carries the honest emulation cost.",
        },
      ],
    },
