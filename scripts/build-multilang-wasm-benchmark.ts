...
await run("clang", [
  ...
  "-o", `${artifactsDir}/olap_c.wasm`, `${rootDir}/benchmarks/multilang-wasm/database-olap-chart/olap.c`,
], "compile olap C");
await run("clang++", [
  "--target=wasm32", "-O3", "-nostdlib",
  "-Wl,--no-entry", "-Wl,--export-all", "-Wl,--initial-memory=1048576",
  "-o", `${artifactsDir}/olap_cpp.wasm`, `${rootDir}/benchmarks/multilang-wasm/database-olap-chart/olap.cpp`,
], "compile olap C++");
await run("rustc", [
  "--target=wasm32-unknown-unknown", "-O", "--crate-type", "cdylib",
  "-C", "strip=symbols",
  "-o", `${artifactsDir}/olap_rs.wasm`, `${rootDir}/benchmarks/multilang-wasm/database-olap-chart/olap.rs`,
], "compile olap Rust");
await run("dart", [
  "compile", "wasm", "--no-source-maps",
  `${rootDir}/benchmarks/multilang-wasm/database-olap-chart/olap.dart`,
  "-o", `${artifactsDir}/olap_dart.wasm`,
...
{
  const jsFn = () => { olapOracle(olapFixture); };
  for (let i = 0; i < 3; i++) jsFn();
  t0 = performance.now();
  ...
  const dartFn = () => {
    olapDart.run(new Uint32Array(olapFixture.buffer.slice(0)), new Uint32Array(560), olapFixture.length);
  };
  for (let i = 0; i < 3; i++) dartFn();
  t0 = performance.now();
...
const report = {
  ...
  workloads: [
    ...
    },
    {
      ...
      description:
        "OLAP aggregation over a 10,000-row, 6-column u32 table with 5 frozen queries " +
