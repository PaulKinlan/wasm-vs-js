...
- `/benchmarks/` contains 38 proposed workloads: P0=12, P1=12, and P2=14. Supplemental implementation coverage is **1/38**: `crypto.file-integrity.v1` runs every registered fixture, schedule, and controlled target. The frozen catalog bytes are unchanged, and `sum-u32` remains a separate harness slice.
...
- The server has no wildcard source, artifact, manifest, or result path handler. Unknown and traversal paths return 404; public non-GET requests return 403, and no public mutation or ingestion route is available.
